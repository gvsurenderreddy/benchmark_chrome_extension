...
function Benchmark() {
  ...
  this.start = function(url) {
    ...
    current_.dnsResults = new Array();
    current_.tcpResults = new Array();
    current_.sslResults = new Array();
    current_.responseResults = new Array();
    current_.pageLoadResults = new Array();
  ...
  this.pageFinished = function(load_times, performance_timing, domNum, depths) {
    ...
    var dnsDuration = performance_timing.domainLookupEnd - performance_timing.domainLookupStart;
    var tcpDuration = performance_timing.connectEnd - performance_timing.connectStart;
    var sslDuration = (performance_timing.secureConnectionStart == 0) ? 0 :
                      performance_timing.connectEnd - performance_timing.secureConnectionStart;
    var responseTime = performance_timing.responseEnd - performance_timing.navigationStart;
    var pageLoadTime = performance_timing.loadEventEnd - performance_timing.navigationStart;
    ...
    current_.dnsResults.push(dnsDuration);
    current_.tcpResults.push(tcpDuration);
    current_.sslResults.push(sslDuration);
    current_.responseResults.push(responseTime);
    current_.pageLoadResults.push(pageLoadTime);
  ...
  port.onMessage.addListener(function(data) {
    if (data.message == "load") {
      ...
      if (benchmark != undefined && benchmark.isRunning()) {
        benchmark.pageFinished(data.loadTimes, data.performanceTiming, data.domNum, data.domDepths);
      }
    }
  });
});
