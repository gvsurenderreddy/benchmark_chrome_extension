...
var METRIC = {"STARTLOAD": 0, "COMMITLOAD": 1, "DOCLOAD": 2, "PAINT": 3,
               ...
               "DNS": 11, "TCP": 12, "SSL": 13, "RESPONSE": 14, "PAGELOAD": 15
             };
...
function getSelectedResults(arr, rowIndex, colIndex) {
  switch (colIndex) {
    ...
    case METRIC.DNS:
      return arr[rowIndex].dnsResults;
    case METRIC.TCP:
      return arr[rowIndex].tcpResults;
    case METRIC.SSL:
      return arr[rowIndex].sslResults;
    case METRIC.RESPONSE:
      return arr[rowIndex].responseResults;
    case METRIC.PAGELOAD:
      return arr[rowIndex].pageLoadResults;  
    case METRIC.REQUESTS:
...
function computeDisplayResults(data) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    obj.dnsMean = Array.avg(obj.dnsResults);


    obj.tcpMean = Array.avg(obj.tcpResults);
    obj.tcpStdDev = Array.stddev(obj.tcpResults);
    obj.tcpStdError = obj.tcpStdDev / Math.sqrt(obj.iterations);

    obj.sslMean = Array.avg(obj.sslResults);
    obj.sslStdDev = Array.stddev(obj.sslResults);
    obj.sslStdError = obj.sslStdDev / Math.sqrt(obj.iterations);



    obj.responseMean = Array.avg(obj.responseResults);
    obj.responseStdDev = Array.stddev(obj.responseResults);
    obj.responseStdError = obj.responseStdDev / Math.sqrt(obj.iterations);


    obj.pageLoadMean = Array.avg(obj.pageLoadResults);
    obj.pageLoadStdDev = Array.stddev(obj.pageLoadResults);
    obj.pageLoadStdError = obj.pageLoadStdDev / Math.sqrt(obj.iterations);
...
function subtractData(data, baseline) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    obj.dnsMean -= obj2.dnsMean;

    obj.tcpMean -= obj2.tcpMean;
    obj.tcpStdDev -= obj2.tcpStdDev;

    obj.sslMean -= obj2.sslMean;
    obj.sslStdDev -= obj2.sslStdDev;

    obj.responseMean -= obj2.responseMean;
    obj.responseStdDev -= obj2.responseStdDev;

    obj.pageLoadMean -= obj2.pageLoadMean;
    obj.pageLoadStdDev -= obj2.pageLoadStdDev;
...
function computeTotals(data) {
  ...
  for (var i = 0; i < count; i++) {
    ...
    totals.dnsMean += obj.dnsMean;
    totals.tcpMean += obj.tcpMean;
    totals.sslMean += obj.sslMean;
    totals.responseMean += obj.responseMean;
    totals.pageLoadMean += obj.pageLoadMean;
  }
}
...
function computeResults(data, baseline) {
  ...
  totals.tcpMean = 0;
  totals.sslMean = 0;
  totals.responseMean = 0;
  totals.pageLoadMean = 0;
  ...
  if (count > 0) {
    ...
    totals.dnsMean /= count;
    totals.tcpMean /= count;
    totals.sslMean /= count;
    totals.responseMean /= count;
    totals.pageLoadMean /= count;
  }
